...
/*
        ...
        this._drawEvent = function(ep) { // row, startCol, widthCols, text
            ...
            if (cache.areas) {
                ...
                for (var i = 0; i < areas.length; i++) {
                    var area = areas[i];
                    var visibility = area.visibility || area.v || "Visible";
        ...
        touch.relativeCoords = function(ev) {
            let coords  = { x: 0, y: 0};
            try {
                const ref = calendar.nav.events;

                const x = ev.touches[0].pageX;
                const y = ev.touches[0].pageY;
                coords = {x: x, y: y};

                const abs = DayPilot.abs(ref);
                coords = {
                    x: x - abs.x, y: y - abs.y, toString: function () {
...
/*
        ...
        img.generate = function(format, options) {
            ...
            // day headers
            DayPilot.list.for(calendar._getColCount()).each(function(x) {
                ...
                board.fillRect(rect, headerBackColor);
                board.rect(rect, borderColor);
                board.text(rectText, text, headerFont, headerColor, "center");
            ...
            DayPilot.list(calendar.rows).each(function(row, y) {
                var height = row.getHeight();
                var left = 0;
                DayPilot.list.for(calendar._getColCount()).each(function(x) {
            ...
            // events
            // var top = rectangles.grid.y;;
            DayPilot.list(calendar.rows).each(function(row) {
                var height = row.getHeight();
                DayPilot.list(row.lines).each(function(line) {
                    DayPilot.list(line).each(function(ep) {
        ...
        img.getWidth = function() {
            ...
            switch (mode) {
                case "viewport":
                case "full":
                    return Math.floor(calendar.nav.top.clientWidth/calendar._getColCount()) * calendar._getColCount();
                default:
        ...
        img.getRectangles = function() {
            ...
            //rectangles.gridContent = {"x": 0, "y": headerHeight, "w": width, "h": height};
    ...
    DayPilotMonth.gMouseUp = function(ev) {
        ...
        if (DayPilotMonth.movingEvent) {
            ...
            // fire the event
            // console.log(e, start.x, start.y, offset, ev, position);
            // debugger;
