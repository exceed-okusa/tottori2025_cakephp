(function ($) {
  ...
  $.fn.wPaint.extend({
     ...
     * pencil
     ****************************************/
    _drawPencilDown: function (e, options, addWidthForEraser) {

      // 変数 addWidthForEraser はエクシード追加
      if (typeof addWidthForEraser === "undefined") {
        addWidthForEraser = 0;
      }
      ...
      if ( options ) {
        this.ctx.lineWidth = parseInt(this.options.lineWidth * options.rate);
      } else {
        this.ctx.lineWidth = parseInt(this.options.lineWidth) + addWidthForEraser;
      }
      
      //draw single dot in case of a click without a move
      this.ctx.beginPath();
      if ( options ) {
        this.ctx.arc(e.pageX, e.pageY, (parseInt(this.options.lineWidth) * options.rate) / 2, 0, Math.PI * 2, true);
      } else {
        this.ctx.arc(e.pageX, e.pageY, (parseInt(this.options.lineWidth) + addWidthForEraser) / 2, 0, Math.PI * 2, true);
      }
      this.ctx.closePath();
      this.ctx.fill();
    ...
    _drawEraserDown: function (e) {
      ...
      this._drawPencilDown(e,null,15);  //  第2引数が消しゴム有効範囲の拡大分。エクシード追加。
    },
