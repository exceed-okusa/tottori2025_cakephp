(function ($) {
  ...
  Paint.prototype = {
    ...
    resize2: function (rate) {
      ...
      this.width = this.$el.width();
      this.height = this.$el.height();
      this.rate = rate; // wPaint.menu.main.js で定義
      ...
      //
      // this['$' + 'canvas']
      //     .attr('width', this.$el.width() + 'px')
      //     .attr('height', this.$el.height() + 'px')

      // this.canvasBg.width = this.$el.width();
      // this.canvasBg.height = this.$el.height();
    ...
    setImage: function (img, ctxType, resize, notUndo) {
      ...
      function loadImage() {
        if ( myImage.width < _this.width) {
          myImage.width = myImage.width * _this.rate;
          myImage.height = myImage.height * _this.rate;
        }
      ...
      if (window.rgbHex(img)) {
        ctx.clearRect(0, 0, this.width * this.rate, this.height * this.rate);
        ctx.fillStyle = img;
        ctx.rect(0, 0, this.width * this.rate, this.height * this.rate);
        ctx.fill();
      }
      else {
        myImage = new Image();
    ...
    getImage: function (withBg) {
      ...
      .attr('width', this.width/this.rate)
      .attr('height', this.height / this.rate);
    ...
    // エクシード追加(実際に動くのは min.js)
    getImageWithoutBg: function () {
    ...
    _callShapeFunc: function (e) {
      ...
      // call drawing func
      if (this[func]) {
        if ( func === '_drawPencilDown' ) {
          this[func].apply(this, [e, this.options]);
        } else {
    ...
    _drawShapeMove: function (e, factor) {
      ...
      const lineWidth = Math.round(this.options.lineWidth * this.rate);
      ...
      e.x = (lineWidth / 2 * factor)+0.2 / this.rate;
      e.y = (lineWidth / 2 * factor) + 0.2 / this.rate;
      e.w = (e.width - lineWidth * factor);
      e.h = (e.height - lineWidth * factor);

      $(this.canvasTemp)
      .css({left: e.left, top: e.top, width: e.width, height: e.height})
      .attr('width', e.width)
      ...
      this.ctxTemp.lineWidth = lineWidth * factor;
    },
    
    _drawShapeUp: function () {
      // 追加 this.canvasTempの幅または高さがゼロの場合に発生する例外(INDEX_SIZE_ERR)を回避
      if ( this.canvasTemp.clientHeight > 0 && this.canvasTemp.clientWidth > 0 && this.canvasTempLeftNew > 0 && this.canvasTempTopNew > 0) {
        try {
