...
class Validator implements ArrayAccess, IteratorAggregate, Countable
{
     ...
     * @param array $originalContext made by exceed-system
     * @return array Array of invalid fields
     */
    public function errors(array $data, $newRecord = true, $originalContext = [])
    {
        $errors = [];
        ...
        foreach ($this->_fields as $name => $field) {
            ...
            $context = compact('data', 'newRecord', 'field', 'providers', 'originalContext');
    ...
    public function addNested($field, Validator $validator)
    {
        ...
        $field->add(static::NESTED, ['rule' => function ($value, $context) use ($validator) {
            ...
            if (empty($this->originalContext)) {
                $this->originalContext = $context['data'];
                $validator->originalContext = $context['data'];
            }
            $errors = $validator->errors($value, $context['newRecord'], $this->originalContext);
    ...
    public function addNestedMany($field, Validator $validator)
    {
        ...
        $field->add(static::NESTED, ['rule' => function ($value, $context) use ($validator) {
            ...
            foreach ($value as $i => $row) {
                ...
                if (!empty($check)) {
                    //////////////////////// by exceed system kaji ////////////////////////
                    for ($j=0; $j < $i; $j++) {
                        if (empty($errors[$j])) {
                            $errors[$j] = ['_dummy'=>['_dummy'=>'_dummy']];
                        }
                    }
                    ///////////////////////////////////////////////////////////////////////
                    $errors[$i] = $check;
