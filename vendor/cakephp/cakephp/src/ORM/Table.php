...
class Table implements RepositoryInterface, EventListenerInterface, EventDispatcherInterface
{
    ...
    public function save(EntityInterface $entity, $options = [])
    {
        ...
        try {
            if (!empty($options['forHistory'])) {
                // insert_user_id, update_user_id は更新しない
            } elseif ( !empty($this->updateUserId) || is_numeric($this->updateUserId) ) {
                if ( $entity->isNew() ) {
                    $entity->insert_user_id = $this->updateUserId;
                }
                $entity->update_user_id = $this->updateUserId;
                // 登録毎にクリアします。
                $this->updateUserId = null;
            } else {
                if ( !empty($_SESSION[PROJECT_PREFIX]['Session']['UserId']) ) {
                    $userId = $_SESSION[PROJECT_PREFIX]['Session']['UserId'];
                    if ( $entity->isNew() ) {
                        $entity->insert_user_id = $userId;
                    }
    ...
    /**
     * （CLOUD版カスタマイズ）<br>
     * 内部更新用（更新ユーザー・日付を更新せずに、内部更新ユーザー・日付を更新する）<br>
     * （※saveに変更があった場合、同じように修正する）
     * @param EntityInterface $entity
     * @param array           $options
     * @return bool|EntityInterface|mixed
     */
    public function internalSave(EntityInterface $entity, $options = [])
    {
        // CLOUD版カスタマイズ
        $options = new ArrayObject($options + [
                                       'atomic' => true,
        ...
        $entity->internal_update_date = new FrozenTime();

        try {
            if ( !empty($_SESSION[PROJECT_PREFIX]['Session']['UserId']) ) {
                $userId = $_SESSION[PROJECT_PREFIX]['Session']['UserId'];
                $entity->internal_update_user_id = $userId;
            }
        } catch ( Exception $e ) {
        }
