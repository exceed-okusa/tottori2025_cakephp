...
//                        echo $token ."\n";
                ...
                // $allTokens = token_get_all("<?php\n" . $code);   // smarty テンプレートファイルの場合は、頭にphp 開始タグをつけないと解析されない・・・
                /*
                $allTokens = $this->replaceCommentToReturn($code);
                $source_array = explode("\n", $allTokens);
                $strs = $this->filterMultibyte($source_array);
                */
                $this->_tokens = [];
//                foreach ($allTokens as $token) {
//                    if (!is_array($token) || ($token[0] !== T_WHITESPACE && $token[0] !== T_INLINE_HTML)) {
...
//                    $token = [0=>308, $s[1], $s[2]];
                    ...
                    if ( !empty($matches) ) {
                        ...
                        foreach ( $matches[1] as $match ) {
                            ...
                            foreach ( $tokens as $token ) {
                                if ( !is_array($token) || ($token[0] !== T_WHITESPACE && $token[0] !== T_INLINE_HTML) ) {
                                    //if ( strpos($s[1], 'CM00402_D11') !== false ) {
                                        //print_r($token[1] . "\n");
                                    //}
                                    $this->_tokens[] = $token;
                                }
    ...
    function replaceCommentToReturn($str) {
        $str = preg_replace_callback('/(\<\!\-\-.*?\-\-\>)/s', array(get_class($this), 'replaceToCRLF'), $str);
        $str = preg_replace_callback('/({\*.*?\*})/s', array(get_class($this), 'replaceToCRLF'), $str);
        return $str;
    }
}