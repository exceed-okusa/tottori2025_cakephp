...
class PaginatorHelper extends Helper
{
    ...
    public function generateUrl(array $options = [], $model = null, $full = false)
    {
        ...
        } elseif ( count($paging['order']) > 1 ) {
            foreach ($paging['order'] as $key => $value ) {
                $param[$key] = $value;
                $url += [
                    'sort' => key($param),
                    'direction' => current($param),
                ];
                break;
            }
        }
...
//            $url['sort'] = $url['direction'] = null;
    ...
    public function counter($options = [])
    {
        ...
        $map = array_map([$this->Number, 'format'], [
            // [0/1]と表示されることを防ぐ為、empty の場合は[固定値：1]を返すように変更
            //'page' => $paging['page'],
            'page' => empty($paging['page']) ? 1 : $paging['page'],
