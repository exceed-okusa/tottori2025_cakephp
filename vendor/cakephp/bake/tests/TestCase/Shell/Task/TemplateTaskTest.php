...
class TemplateTaskTest extends TestCase
{
    ...
    public function testBakeIndex()
    {
        ...
        $this->Task->expects($this->at(0))->method('createFile')
            ->with(
                $this->_normalizePath(APP . 'Template/TemplateTaskComments/excel_download.ctp')
            );
    ...
    public function testBakeTreeNoLftOrRght()
    {
        ...
        $this->Task->expects($this->at(1))
            ->method('createFile')
            ->with(
                $this->_normalizePath(APP . 'Template/CategoryThreads/excel_download.ctp'),
                $this->logicalNot(
                    $this->logicalAnd(
                        $this->stringContains('rght'),
                        $this->stringContains('lft')
    ...
    public function testMainWithPluginName()
    {
        ...
        $filename = $this->_normalizePath(
            APP . 'Plugin/TestTemplate/src/Template/TemplateTaskComments/excel_download.ctp'
        );
    ...
    public function testMainWithControllerFlag()
    {
        ...
        $templates = ['excel_download.ctp', 'view.ctp', 'add.ctp', 'edit.ctp'];
    ...
    public function testMainWithControllerAndAdminFlag()
    {
        ...
        $templates = ['excel_download.ctp', 'add.ctp'];
        foreach ($templates as $i => $template) {
            $this->Task->expects($this->at($i))->method('createFile')
                ->with(
