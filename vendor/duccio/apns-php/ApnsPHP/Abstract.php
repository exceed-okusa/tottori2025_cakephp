...
abstract class ApnsPHP_Abstract
{
	 ...
	 */
	public function __construct($nEnvironment, $sProviderCertificateFile, $nProtocol, $keyId, $teamId, $bundleId)
	{
	...
	{
        $keyfile = "file://" . $this->_sProviderCertificateFile; //absolute path
        $key = openssl_pkey_get_private($keyfile);

        if ( $key === false ) {
            throw new ApnsPHP_Exception(openssl_error_string());
        }

        $header = ['alg' => 'ES256', 'kid' => $this->_keyId];
        $claims = ['iss' => $this->_teamId, 'iat' => time()];

        $header_encoded = $this->_base64($header);
        $claims_encoded = $this->_base64($claims);
...
//		$this->_log($opt);
		...
		if (!curl_setopt_array($this->_hSocket, $opt)) {
			throw new ApnsPHP_Exception(
				"Unable to initialize HTTP/2 backend."
			);
		}
	...
	protected function _base64($data)
    {
