...
use App\Model\ControllerModel\BaseModel;
use App\Utils\Enum;
...
class BaseSql
{
    ...
    public function __construct(BaseModel $model)
    {
        ...
        $this->defaultDb = $this->controller->getDbName('default');
    }
    ...
    /**
     * 企業の前回締日の翌日・次回締日を格納した配列を返します。
     *
     * @since 1.0.0 j-yasuda  First time this was introduced.
     * @param $targetDate
     * @return array
     *
     * return
     *     [
     *         [ 'dateFrom' => (FrozenDate), 'dateTo' => (FrozenDate) ],
     *         [ 'dateFrom' => (FrozenDate), 'dateTo' => (FrozenDate) ],
     *               .
     *               .
     *               .
     *     ]
     */
    protected function getCompanySalesClosingScope($targetDate)
    {
        /* @var \App\Model\Table\CompaniesTable $tableRegistryCompanies */
        $tableRegistryCompanies = TableRegistry::get('Companies');
        return $tableRegistryCompanies->getSalesClosingScope($targetDate, $this->targetCompanyId);
    }

    /**
     * 各店舗の前回締日の翌日・次回締日を格納した配列を返します。
     *
     * @since 1.0.0 s-kaji  First time this was introduced.
     * @param $targetDate
     * @return array
     *
     * return
     *     [
     *         [ 'storeId' => (int), 'dateFrom' => (FrozenDate), 'dateTo' => (FrozenDate) ],
     *         [ 'storeId' => (int), 'dateFrom' => (FrozenDate), 'dateTo' => (FrozenDate) ],
     *               .
     *               .
     *               .
     *     ]
     */
    protected function getSalesClosingScope($targetDate)
    {
        return $this->Stores->getSalesClosingScope($targetDate, $this->targetCompanyId, $this->targetStoreId);
    }

    /**
     * @return string
     * @deprecated formatStaffCodesを使う
     */
    protected function getConditionStaffCodes()
    {
        $conditionStaffCode = '';

        if (!empty($this->params['onlyStaffCodes'])) {
            foreach ($this->params['onlyStaffCodes'] as $staffCode) {
                if (!empty($conditionStaffCode)) {
                    $conditionStaffCode .= ",";
                }
                $conditionStaffCode .= "'". $staffCode. "'";
            }
            $conditionStaffCode = "IN ( ". $conditionStaffCode. " )";
        } else if (!empty($this->params['excludeStaffCodes'])) {
            foreach ($this->params['excludeStaffCodes'] as $staffCode) {
                if (!empty($conditionStaffCode)) {
                    $conditionStaffCode .= ",";
                }
                $conditionStaffCode .= "'". $staffCode. "'";
            }
            $conditionStaffCode = "NOT IN ( ". $conditionStaffCode. " )";
        }

        return $conditionStaffCode;
    }

    /**
     * @param $arrayStaffCodes
     * @return string
     */
    protected function formatStaffCodes($arrayStaffCodes)
    {
        $array = array_map(function($a){return "'$a'";}, $arrayStaffCodes);
        $str = implode(',', $array);
        return $str;
    }

    /**
     * @param $arrayCustomerCodes
     * @return string
     */
    protected function formatCustomerCodes($arrayCustomerCodes)
    {
        $array = array_map(function($a){return "'$a'";}, $arrayCustomerCodes);
        $str = implode(',', $array);
        return $str;
    }
}
