...
class BaseForm extends Form
{
    ...
    private $targetValidate = 'default';
    ...
    {
        $suffix = strtoupper(substr($this->targetValidate, 0, 1)). substr($this->targetValidate, 1);
        $method = 'validation'. $suffix;
        return $this->{$method}($validator);
    }

    public function changeValidate($targetValidate)
    {
        $this->targetValidate = $targetValidate;
    }

    // バリデーションチェック実行時の関数を上書き。エラー時に notice ログを吐き出すようにする。
    ...
    protected function checkNotEmpty(Validator $validator, array $checkNotEmpty)
    {
    ...
    protected function dateCompare($context, $keyFrom, $keyTo)
    {
        if (!Validation::date($context['data'][$keyFrom], ['ymd']) || !Validation::date($context['data'][$keyTo], ['ymd'])) {
            return true;  // 日付型チェックのほうで引っ掛ける
        }

        return ( new FrozenDate($context['data'][$keyFrom]) )->format('Ymd') <= ( new FrozenDate($context['data'][$keyTo]) )->format('Ymd');
    }
}