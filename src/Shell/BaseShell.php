...
class BaseShell extends Shell
{
    ...
    public function initialize()
    {
        ...
        $this->controller->shell = $this;  // Component で使う用
    }

    public function startup(){
        // ウェルカムメッセージを表示しないようoverride

        // アイドル時間の変更
        $this->Common->setMysqlValueWaitSeconds(8 * 60 * 60);
    }

    public function __get($name)
    {
        if (
            property_exists($this->controller, $name)
            || class_exists('App\Model\Table\\'. $name. 'Table')
            || class_exists('Cake\Controller\Component\\'. $name. 'Component')
            || class_exists('App\Controller\Component\\'. $name. 'Component')
        ) {
    ...
    public function checkAndRetryConnection()
    {
        try {
     ...
     * @throws \Exception
     */
    protected function execute($queries)
    {
        $data = [];
        ...
        foreach ($queries as $query) {
            ...
            if ($i !== $count) {
                ...
                // 最後のひとつ
                $data = $this->connection->query($query)->fetchAll('assoc');
            }
        }
    ...
    private function executeMultiQuery($query) {
        ...
        try {
            ...
            do {
                $i++;
            } while( $stmt->nextRowset() ); // nextRowset()は exceed 作成

        } catch( \Exception $e ) {
            $arrayQueries = explode(';', $query);
            throw new \Exception( $i. "番目のクエリ：". $stmt->errorInfo()[2]. "\r\n". $arrayQueries[$i-1] );
        }
    }

    public function startPercentage($all)
    {
        $str = $this->Common->startPercentage($all);
        $this->out($str);
    }

    public function outPercentage()
    {
        $str = $this->Common->outPercentage();
        if (!empty($str)) {
            $this->out($str);
        }
    }
     ...
     * @param $message
     */
    public function outAndLogShell($message)
    {
        $this->out(( new FrozenTime() )->format('H:i:s'). ' '. $message);
        $this->logShell($message, 1);
    }
}
